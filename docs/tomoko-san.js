$(function() {
	...
	$.ajax({
		type: 'GET',
		    url: 'https://script.google.com/macros/s/AKfycbzzOlPFoFwQMeW0P_loMTGTEiD06H5Rn553eTo7/exec',
		    dataType: 'jsonp',
		    jsonpCallback: 'jsondata',
		    success: function(json){
		    var r = new Array(0);
		    var review = new Array(0);
		    var txt = new Array(0);
		    var title = new Array(0);
		    var data2;
		    //console.log(json[0].radius, json[0].review, json[0].txt, json[0].title)
			//json[0].review), json[0].txt, json[0].title);
		    data2 = json;

	// var data = json;
		    //var data = data2;
		    console.log(json[0].radius)
		...
		// nodesDataにデータをpush
		// x座標と、y座標はランダム
		for (var i = 0, d = data2.length; i < d; i++) {
		    nodesData.push({
			    x: xRamdom,
				y: yRamdom,
				r: 50, //data2[i].radius,
				rev: data2[i].evaluation,
				txt: data2[i].comment,
				title: data2[i].comment, //"test", //data2[i].title
				});
		}
...
// SpreadSheetへデータを送信
function sendData_GAS(r, review, txt, title) {
    var sendData = "radius="+ r + "&review="+review+ "&txt="+txt+ "&title="+title;
